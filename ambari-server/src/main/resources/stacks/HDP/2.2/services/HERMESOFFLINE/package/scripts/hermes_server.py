...
SERVICE_NAME = "HermesServer"


class HermesServer(Script):
    ...
    def configure(self, env):
        ...
        util.load_command_script(env, 'start_hermesserver.sh', 'start_hermesserver.sh.j2')

    def start(self, env):
        import params
        env.set_params(params)
        start_hermes_server_cmd = format("sudo bash -x {start_service_script} {start_hermes_server_script}")
        is_success = util.command_exe(start_hermes_server_cmd, SERVICE_NAME)
        if not is_success:
